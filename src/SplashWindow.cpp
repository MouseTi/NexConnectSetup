...
bool SplashWindow::Initialize() {
    ...
    // Create font - larger "Nex" with letter spacing like SplashScreen.cpp
    m_font = CreateFontW(
    ...
    switch (uMsg) {
        ...
        case WM_TIMER:
            if (wParam == WM_TIMER_ANIM) {
                m_spinPhase -= 0.012;
                if (m_spinPhase < 0.0) m_spinPhase = 1.0;
                InvalidateRect(m_hwnd, nullptr, FALSE);
            }
            return 0;
...
void SplashWindow::OnPaint() {
    ...
    // Draw "Nex" text in top-left (larger, with letter spacing)
    SetBkMode(hdcMem, TRANSPARENT);
    SetTextColor(hdcMem, RGB(255, 255, 255));
    HFONT oldFont = (HFONT)SelectObject(hdcMem, m_font);
    RECT textRect = {16, 14, clientRect.right, 50};
...
void SplashWindow::DrawLoading(HDC hdc, const RECT& clientRect) {
    ...
    float sweepAngle = 90.0f; // Positive = clockwise on screen (right-to-left)
